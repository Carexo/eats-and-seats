...
import { useDeleteOpinion, useOpinionsByUserId, useUpdateOpinion } from '../../api/queries/opinions';
...
const UserOpinionsPage: React.FC = () => {
    ...
    const { data: opinions = [], isLoading, refetch } = useOpinionsByUserId(username);
    const { notificationSend } = useActions();
    const deleteOpinion = useDeleteOpinion(notificationSend);

    const [editingOpinion, setEditingOpinion] = useState<IExtendOpinion | null>(null);
    const [newDescription, setNewDescription] = useState('');
    const [newRating, setNewRating] = useState(0);

    const updateOpinion = useUpdateOpinion(notificationSend, editingOpinion?.opinionID || '', {

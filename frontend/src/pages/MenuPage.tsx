...
import { Card, Col, Row, Typography, Spin, Space, Button, Slider, Input } from "antd";
...
const MenuPage = () => {
  ...
  useEffect(() => {
      axios.get('http://localhost:3000/menu')
          .then(response => {
              ...
              setLoading(false);
              const maxPrice = Math.max(...response.data.map((dish: Dish) => dish.price));
  ...
  useEffect(() => {
      filterDishes();}, [selectedCategory, priceRange, searchTerm]);

  const filterDishes = () => {
      let filtered = dishes;
      if (selectedCategory !== "All") {
          filtered = filtered.filter(dish => dish.category && dish.category.toLowerCase() === selectedCategory.toLowerCase());
      }
          ...
          {showFilters && (
              <div style={{ marginBottom: "20px", textAlign: "center" }}>
                  ...
                  <div style={{ marginTop: "20px" }}>
                      ...
                      />
                      <Text>Zakres: {priceRange[0]} zł - {priceRange[1]} zł</Text>
                  </div>
              </div>
          )}

          {loading ? (
              <Space size="middle" style={{display: "flex", justifyContent: "center", marginTop: "50px"}}>
                  <Spin size="large"/>
              </Space>
          ) : (

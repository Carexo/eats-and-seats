...
const DishDetails: React.FC = () => {
    ...
    useEffect(() => {
        const fetchDishDetails = async () => {
            try {
                const dishResponse = await axios.get<Dish>(`http://localhost:3000/api/dishes/${id}`);
    ...
    if (error) {
        return <Alert message="Błąd" description={error} type="error" showIcon style={{ margin: '20px' }} />;
    }

    if (!dish) {
        return <Alert message="Brak danych" description="Nie znaleziono szczegółów dania." type="warning" showIcon style={{ margin: '20px' }} />;
    }

    return (
        <Card
            className="dish-card"
            style={{ maxWidth: 700, margin: '50px auto', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.2)' }}
            cover={
                <img
                    alt={dish.name}
                    src={dish.image}
                    style={{ height: 400, objectFit: 'cover', borderRadius: '8px 8px 0 0' }}
                />
            }
        >
            <div className="dish-header" style={{ textAlign: 'center', marginBottom: 20 }}>
